...
def populate_history_table(
    ...
    ft_cte_names = tuple(
        f"{fk_table_name}_latest"
        for fk_table_name, fk_pk_name in (foreign_tables or tuple())
    )
    ...
    ft_cte_defns = tuple(
        f"""
        {cte_name} AS (
            SELECT {fk_pk_name}, max({hx_id_name(fk_table_name)}) value 
            FROM {hx_table_name(fk_table_name)} 
            GROUP BY {fk_pk_name}
        )        
        """
        for cte_name, (fk_table_name, fk_pk_name) in zip(
    ...
    ft_where_conditions = tuple(
        f"main.{fk_pk_name} = {cte_name}.{fk_pk_name}"
        for cte_name, (fk_table_name, fk_pk_name) in zip(
            ft_cte_names, foreign_tables or tuple()
        )
    )
