...
def check_matviews(
    connection: Connection, matview_defns, schema_name, matviews_present
):
    """
    Test helper.
    ...
    if matviews_present:
        ...
        names = get_schema_item_names(connection, "tables", schema_name=schema_name)
        assert (
            names & matview_names == set()
        ), "matview(s) should not be present as table(s)"
        names = get_schema_item_names(connection, "matviews", schema_name=schema_name)
