...
class CreateStoredProcedureOp(MigrateOperation):
    ...
    def __init__(self, name, sqltext, schema=None):
        self.name = name
        self.sqltext = sqltext
        self.schema = schema

    @classmethod
    def create_stored_procedure(cls, operations, name, sqltext, **kw):
        """Issue a "CREATE SEQUENCE" instruction."""

        op = CreateStoredProcedureOp(name, sqltext, **kw)
...
class DropStoredProcedureOp(MigrateOperation):
    ...
    @classmethod
    def drop_stored_procedure(cls, operations, name, sqltext, **kw):
        """Issue a "DROP SEQUENCE" instruction."""

        op = DropStoredProcedureOp(name, sqltext, **kw)
        return operations.invoke(op)
