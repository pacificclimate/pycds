...
- Define bookmarks $L_1$ and $L_2$ to demarcate (bracket) the release.
...
| ? `network_id` | `int`       | FK `meta_network`. Use is to distinguish bookmarks for one network from another, and allow a simple, natural name in common, such as 'QA'. Normalizes a common use case, I think. Tempting to make nullable, but caution, nullable columns have frequently been abused in CRMP. |
| `mod_user`     | `text`      |                                                                                                                                                                                                                                                                                 |
| `mod_time`     | `timestamp` |                                                                                                                                                                                                                                                                                 |
...
2. Is `network_id` actually needed? A network is implied by a bookmark's association to items. This is also true of variables, but they too have a sdirect network association (`network_id`) that is not strictly necessary. That fact inspired the idea of having a direct association for bookmarks as well. The slight over-specificity may be offset by the utility of easily segregating these things (variables, bookmarks) by network.
