...
def test_vars_content(sesh_with_large_data):
    ...
    old_expected_rows_sh = {(row[2], row[3]) for row in old_expected_rows}
    ...
    print("result_rows")
    for row in q.all():
        if (row.station_id, row.history_id) in old_expected_rows_sh:
            print(
                f'("{str(row.min_obs_time)}", ',
                f'"{str(row.max_obs_time)}", ',
                f'{row.station_id}, ',
                f'{row.history_id}),',
            )
