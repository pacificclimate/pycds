...
@fixture
def sesh_with_other_network_and_climatology_variables(
    pycds_sesh, other_network, other_climatology_variables
):
...
@fixture
def sesh_with_climate_baseline_variables(
...
@fixture
def sesh_with_station_and_history_records(
    sesh_with_climate_baseline_variables, stations, histories
):
    for sesh in add_then_delete_objs(
        sesh_with_climate_baseline_variables, stations + histories
    ):
...
def describe_get__or__create__pcic__climate__variables__network():
    def it_creates_the_expected_new_network_record(pycds_sesh):
        ...
        results = pycds_sesh.query(Network).filter(
...
def describe_create__pcic__climate__baseline__variables():
    def it_returns_the_expected_variables(
        sesh_with_other_network_and_climatology_variables
    ):
    ...
    def it_creates_temperature_variables(
        ...
        assert result.short_name == "air_temperature {}".format(
            result.cell_method
        )
        assert (
            result.cell_method
            == "t: {} within days t: mean within months t: mean over years".format(
                keyword
            )
        )
        assert (
            result.description
            == "Climatological mean of monthly mean of {} daily temperature".format(
                keyword
            )
        )
    ...
    def it_creates_precip_variable(sesh_with_climate_baseline_variables):
        ...
        assert (
            result.short_name
            == "lwe_thickness_of_precipitation_amount {}".format(
                result.cell_method
            )
        )
    ...
    def it_creates_no_more_than_one_of_each(pycds_sesh):
        ...
        get_or_create_pcic_climate_baseline_variables(sesh)
        results = sesh.query(Variable).filter(
            Variable.name.like("%_Climatology")
        )
...
def describe_load__pcic__climate__baseline__values():
    def describe_with_station_and_history_records():
        ...
        def describe_with_an_invalid_climate_network_or_variable_name():
            @mark.parametrize(
                "network_name, var_name",
                [
                    ("foo", "Tx_Climatology"),
                    (pcic_climate_variable_network_name, "foo"),
                ],
            )
            def it_throws_an_exception(
                sesh_with_station_and_history_records, network_name, var_name
            ):
                sesh = sesh_with_station_and_history_records
                with raises(ValueError):
                    load_pcic_climate_baseline_values(
                        sesh, var_name, [], network_name=network_name
                    )
        ...
        def describe_with_valid_network_and_variable_names():
            ...
            def describe_with_no_missing_data():
                ...
                def source(request, stations):
                    ...
                    for station in stations:
                        if request.param in [
                            ...
                            values = [
                                str(100 * station.id + 2 * month).encode(
                                    "ascii"
                                )
                                for month in range(1, 13)
                            ]
                ...
                def it_correctly_converts_and_loads_values_into_the_database(
                    ...
                    n_lines_added, n_values_added, n_lines_errored, n_lines_excluded, n_lines_skipped = load_pcic_climate_baseline_values(
                        sesh, var_name, source, exclude
                    )
                    ...
                    for station in stations:
                        ...
                        for i, value in enumerate(station_values):
                            ...
                            if var_name in ["Tx_Climatology", "Tn_Climatology"]:
                                ...
                                assert (
                                    value.datum == 100 * station.id + 2 * month
                                )
            ...
            def describe_with_a_station_with_missing_data_for_some_months():
                ...
                def it_loads_only_non_absent_values(
                    ...
                    assert set([sv.time.month for sv in station_values]) == {
                        2,
                        3,
                        4,
                        5,
...
def describe_verify__baseline__network__and__variables():
    ...
    def describe_with_baseline_network_missing_a_variable():
        @fixture
        def sesh_with_1_var_missing(
            request, sesh_with_climate_baseline_variables
        ):
            sesh = sesh_with_climate_baseline_variables
            variable = next(
                var
                for var in get_or_create_pcic_climate_baseline_variables(sesh)
        ...
        @mark.parametrize(
            "missing_var, sesh_with_1_var_missing",
            [
                ("Tx_Climatology", "Tx_Climatology"),
                ("Tn_Climatology", "Tn_Climatology"),
                ("Precip_Climatology", "Precip_Climatology"),
    ...
    def describe_with_baseline_network_and_variables():
        def describe_with_all_as_expected():
            def it_doesnt_raise_an_exception(
                sesh_with_climate_baseline_variables
            ):
                try:
                    finished = verify_baseline_network_and_variables(
                        sesh_with_climate_baseline_variables
                    )
        ...
        def describe_with_incorrect_attributes_in_variables():
            ...
            def it_raises_an_exception(
                ...
                variable = next(
                    var
                    for var in get_or_create_pcic_climate_baseline_variables(
                        sesh
                    )
...
def describe_verify__baseline__values():
    def describe_with_baseline_network_and_variables():
        def describe_with_no_values():
            @mark.parametrize("var_name", climatology_var_names)
            def it_raises_an_exception(
                sesh_with_climate_baseline_variables, var_name
            ):
                sesh = sesh_with_climate_baseline_variables
                with raises(AssertionError) as excinfo:
        ...
        def describe_with_valid_climate_baseline_values_in_database():
            @fixture
            def sesh_with_climate_baseline_values(
                sesh_with_station_and_history_records, histories
            ):
                sesh = sesh_with_station_and_history_records
                variables = (
                    sesh.query(Variable)
                ...
                derived_values = [
                    DerivedValue(
                        time=datetime.datetime(
                            2000, month, monthrange(2000, month)[1], 23
                        ),
                        datum=float(month),
                        variable=variable,
                        history=histories[h],
            ...
            def describe_with_compatible_value_expectations():
                    ...
                    [
                        ...
                        [
                            ...
                            },
                            {
                                ...
                                "values": [
                                    (m if m % 2 else None) for m in range(1, 13)
                                ],
                            },
                        ],
                    ],
                ...
                def it_succeeds(
                    sesh_with_climate_baseline_values,
                    var_name,
                    expected_stations_and_values,
                ):
                    sesh = sesh_with_climate_baseline_values
                    try:
                        finished = verify_baseline_values(
                            sesh, var_name, 2, expected_stations_and_values
                        )
                    except AssertionError:
                        fail("Unexpected AssertionError from verify")
                    assert finished
            ...
            def describe_with_incompatible_value_expectations():
                ...
                @mark.parametrize(
                    ...
                    [
                        ...
                        (
                            "100",
                            list(range(1, 5)) + [99] + list(range(6, 13)),
                            "datum",
                        ),
                        (
                            "200",
                            [(m if m % 2 == 0 else None) for m in range(1, 13)],
                            "datum",
                        ),
                    ],
                )
