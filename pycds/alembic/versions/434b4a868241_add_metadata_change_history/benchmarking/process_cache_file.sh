...
do
  echo "Batching $f"
  echo "LD_LIBRARY_PATH=$LD_LIBRARY_PATH PATH=$CRMP_PATH crmprtd_process -N ec -c postgresql://dbtest02.pcic.uvic.ca:5432/crmp_hx -L ~/logging.yaml --log_filename ~/ec/logs/crmp_dbtest02_bc_hourly_json.log < $f"| batch
done
