...
def load_pcic_climate_baseline_values(
    session,
    var_name,
    lines,
    exclude=[],
    ...
    def parse_line(line):
        field_values = struct.unpack(
            field_format, bytes(line.rstrip("\n").encode("ascii"))
        )
...
def expect_value(what, value, expected):
    assert value == expected, '{}: expected "{}", got "{}"'.format(
        what, expected, value
    )
...
def verify_baseline_network_and_variables(session):
    ...
    assert networks.count() == 1, 'Network "{}" not found in database'.format(
...
def verify_baseline_values(
    ...
    for expected_stn_and_values in expected_stations_and_values:
        ...
        stn_dvs = (
            derived_values.filter(Station.native_id == station_native_id)
            .order_by(DerivedValue.time)
            .all()
        )
        expected_values = expected_stn_and_values["values"]
        expect_value(
            'Variable "{}", Station "{}" value count'.format(
                var_name, station_native_id
            ),
            len(stn_dvs),
            12 - expected_values.count(None),
        )
        stn_idx = 0
        month = 1
        for expected_value in expected_values:
            if expected_value is not None:
                expect_value(
                    "Variable {}, Station {}, Month {} datum".format(
                        var_name, station_native_id, month
                    ),
                    stn_dvs[stn_idx].datum,
                    expected_value,
                )
                stn_idx += 1
            month += 1
