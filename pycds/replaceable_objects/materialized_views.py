...
from pycds.materialized_view_helpers import (
    ...
    create_native_materialized_view,
    drop_native_materialized_view,
)
...
@Operations.register_operation("create_native_materialized_view", "invoke_for_target")
@Operations.register_operation("replace_native_materialized_view", "replace")
class CreateNativeMaterializedViewOp(ReversibleOp):
    def reverse(self):
        return DropNativeMaterializedViewOp(self.target, schema=self.schema)


@Operations.register_operation("drop_native_materialized_view", "invoke_for_target")
class DropNativeMaterializedViewOp(ReversibleOp):
    def reverse(self):
        return CreateNativeMaterializedViewOp(self.target, schema=self.schema)
...
@Operations.implementation_for(CreateNativeMaterializedViewOp)
def create_native_materialized_view_op(operations, operation):
    operations.execute(
        create_native_materialized_view(
            target_name(operation),
            operation.target.__selectable__.compile(
...
@Operations.implementation_for(DropNativeMaterializedViewOp)
def drop_native_materialized_view_op(operations, operation):
    operations.execute(drop_native_materialized_view(target_name(operation)))
