...
def main_table_name(collection_name: str, schema=schema_name) -> str:
    return qualified_name(collection_name, schema=schema)


def hx_table_name(collection_name: str, **kwargs) -> str:
    return f"{main_table_name(collection_name, **kwargs)}_hx"
...
def create_history_table(collection_name: str, foreign_tables: list[tuple[str, str]]):
    ...
    columns = ", ".join(
        (
            f"  LIKE {main_table_name(collection_name)} INCLUDING ALL EXCLUDING INDEXES",
...
def create_primary_table_triggers(collection_name: str, prefix: str = "t100_"):
    ...
    op.execute(
        ...
        f"    ON {main_table_name(collection_name)} "
        f"    FOR EACH ROW "
