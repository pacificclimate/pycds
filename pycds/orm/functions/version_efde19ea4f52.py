...
getstationvariabletable = ReplaceableFunction(
    ...
    AS $BODY$
        query = (
            ...
            "FROM {schema_name}.meta_vars JOIN {schema_name}.meta_station ON {schema_name}.meta_vars.network_id = {schema_name}.meta_station.network_id " 
            "WHERE ARRAY['" + ("climatology" if climo else "observation") + "'::text] <@ {schema_name}.variable_tags({schema_name}.meta_vars.*) " 
