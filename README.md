...
- [Replaceable objects](#replaceable-objects)
  - [Intro](#intro)
...
- [Testing](#testing)
  - [Project unit tests](#project-unit-tests)
    ...
    - [Running tests locally with the `pycds-local-pytest` Docker image](#running-tests-locally-with-the-pycds-local-pytest-docker-image)
...
#### Running tests locally with the `pycds-local-pytest` Docker image

It is difficult to establish an appropriate run-time environment on
a dev machine for running tests. To solve this problem, there is a 
Docker image [`pcic/pycds-local-pytest`](docker/local-pytest/README.md)
for that purpose.
It establishes the same environment our CI provides, and when
run in interactive mode as described in the 
[README](docker/local-pytest/README.md), it is no different than running tests
directly on the local machine.
This is far and away the easiest way to run tests locally. 
